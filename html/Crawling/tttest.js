...
var resultImg='';
var db=function(url,parameter,func){
	...
	client.fetch(url,parameter,function(err,$,res,body){
		...
		resultImg=$(imgList[0]).attr('data-src');

		JString={'recipeId': '2', 'recipeName': $(aList[0]).text(), 'recipeUrl': search_url,'recipeImg':resultImg};
			...
			insertRecipe(JString.recipeId,JString.recipeName, JString.recipeUrl,JString.recipeImg,
				function(err,inserted){
					...
					if(inserted){
						...
						var recipeName=inserted.recipeName;
						console.log('추가한 레코드의 이름:'+recipeName);
					}
					else{
						console.log('추가 실패');
					}
...
var ee=scrapContent('http://www.naver.com/');
...
//web crwaling 함수 호출!
db('http://terms.naver.com/search.nhn',{query:'미역국'},function(url){
